...
def hotelling_reject(seed,n=300):
    np.random.seed(seed)
    df1 = b2(n)
    y = df1['Y'].to_numpy()
    w = df1['Wbar'].to_numpy()
    ...
    if model2fit.message == "Optimization terminated successfully.":
        ...
        filename = path + '/dir37/' + str(seed)+'.csv'
...
if __name__ == '__main__':
    ...
    df_out.to_csv("/work/qingyang/simu_beta/37simulation_N300_bootstrap_unknown_variance_linear.csv",index=False)
