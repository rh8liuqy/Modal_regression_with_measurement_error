...
def b2(sample_size,m=40,repeated_measure=3,SigmaW=np.sqrt(1.2)):
    beta0 = 1
    beta1 = 1
    beta2 = 1
    beta3 = 1
...
def hotelling_reject(seed,n=400):
    np.random.seed(seed)
    df1 = b2(n)
    y = df1['Y'].to_numpy()
    w = df1['Wbar'].to_numpy()
    ...
    model1 = pyb.reg(x=np.column_stack([z,w]), y=y,initial=[40,1,1,1])
    ...
    if model2fit.message == "Optimization terminated successfully.":
        ...
        filename = path + '/dir38/' + str(seed)+'.csv'
...
if __name__ == '__main__':
    ...
    df_out.to_csv("/work/qingyang/simu_beta/38simulation_N400_bootstrap_unknown_variance_linear.csv",index=False)
