...
def b2(sample_size,m=3,repeated_measure=3,SigmaW=np.sqrt(3)):
    ...
    for i in range(sample_size):
        ...
        U1 = np.random.laplace(0,np.sqrt(3/2),1)
        U2 = np.random.laplace(0,np.sqrt(3/2),1)
        U3 = np.random.laplace(0,np.sqrt(3/2),1)
        W1 = X1 + U1
        W2 = X1 + U2
        W3 = X1 + U3
...
if __name__ == '__main__':
    ...
    df_out.to_csv("20simulation_N200_corrected_variance_unknown_robust.csv",index=False)
