...
def hotelling_reject(seed,n=100):
    np.random.seed(seed)
    df1 = b2(n)
    y = df1['Y'].to_numpy()
    w = df1['Wbar'].to_numpy()
    ...
    if model2fit.message == "Optimization terminated successfully.":
        ...
        filename = path + '/dir31/' + str(seed)+'.csv'
...
if __name__ == '__main__':
    ...
    df_out.to_csv("/work/qingyang/simu_beta/31simulation_N100_bootstrap_unknown_variance.csv",index=False)
