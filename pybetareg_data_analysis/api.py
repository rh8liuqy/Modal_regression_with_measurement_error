...
"""
import numpy as np
import pybetareg_data_analysis.main

def reg(x,y,initial="NA",column_names="NA",link="logit"):
    x = np.array(x)
    y = np.array(y)
    out = pybetareg_data_analysis.main.betamodal(x,y,initial,column_names,link)
    return out
...
def reg_measurement_error(y,w,z,sigmaw,initial,
    ...
    out = pybetareg_data_analysis.main.betamodal_measurement_error(y,w,z,
                                                     sigmaw,
                                                     initial,
                                                     monte_carlo_size,
                                                     repeated_measure,
